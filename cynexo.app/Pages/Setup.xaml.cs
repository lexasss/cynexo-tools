...
public partial class Setup : Page, IPage<Navigation>
{
    ...
    readonly Communicator.CommPort _sniff0 = Communicator.CommPort.Instance;
    ...
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetVerbose(tbnSetVerbose.IsChecked ?? false)));
    }
    ...
    private void SetVerboseLCD_Toggled(object sender, RoutedEventArgs e)
    {
        if (IsLoaded)
        {
            HandleResponse(_sniff0.Send(Communicator.Command.SetVerboseLCD(tbnSetVerboseLCD.IsChecked ?? false)));
        }
    }

    private void SetCAChannel_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetCAChannel((int)sldSetCAChannel.Value)));
    }

    private void OpenCloseValves_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetAllSolenoidValves(cmbOpenCloseValves.SelectedIndex == 0)));
    }
    ...
    private void Calibrate_Click(object sender, RoutedEventArgs e)
    {
        ...
        HandleResponse(_sniff0.Send(Communicator.Command.SetFlow(kvs.ToArray())));
    }

    private void CalibrateManually_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetCAChannel((int)sldCalibrateManually.Value)));
    }

    private void StopCalibration_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.StopCalibration));
    }

    private void TestDelay_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetCAChannel((int)sldTestDelay.Value)));
    }

    private void SetChannel_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetCAChannel((int)sldSetChannel.Value)));
    }

    private void ReadFlow_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.ReadFlow));
    }

    private void SetValve_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetValve(cmbSetValve.SelectedIndex == 0, chkSetValveTrigger.IsChecked ?? false)));
    }

    private void SetMotorDirection_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.SetMotorDirection(cmbSetMotorDirection.SelectedIndex == 0)));
    }

    private void RunMotorSteps_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.RunMotorSteps((int)sldRunMotorSteps.Value)));
    }
    ...
    private void OpenValve_Click(object sender, RoutedEventArgs e)
    {
        if (int.TryParse(txbOpenValve.Text, out int ms))
        {
            ...
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValve(ms, chkOpenValveTrigger.IsChecked ?? false)));
            }
            else if (rdbOpenValveNoCA.IsChecked == true)
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValveWithoutCleanAir(ms, chkOpenValveTrigger.IsChecked ?? false)));
            }
            else if (rdbOpenValveNoCF.IsChecked == true)
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValveWithoutConstantFlow(ms, chkOpenValveTrigger.IsChecked ?? false)));
            }
        }
    }
    ...
    private void SetTriggerOutDelay_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            HandleResponse(_sniff0.Send(Communicator.Command.SetTriggerOutDelay(ms)));
        }
    }
    ...
    private void SetTriggerOutDuration_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            HandleResponse(_sniff0.Send(Communicator.Command.SetTriggerOutDuration(ms)));
        }
    }

    private void TestTriggerOut_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.OutTrigger));
    }

    private void TestTriggerIn_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.InTrigger));
    }

    private void LoopTrigger_Click(object sender, RoutedEventArgs e)
    {
        HandleResponse(_sniff0.Send(Communicator.Command.LoopTrigger));
    }

    private void OpenValveOn_Click(object sender, RoutedEventArgs e)
    {
        if (int.TryParse(txbOpenValveOnDuration.Text, out int duration) && int.TryParse(txbOpenValveOnDelay.Text, out int delay))
        {
            int channel = (int)sldOpenValveOn.Value;
            if (cmbOpenValveOn.SelectedIndex == 0)
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValveOnInhale(channel, duration, delay, chkOpenValveOnSecondTrigger.IsChecked ?? false)));
            }
            else
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValveOnExhale(channel, duration, delay, chkOpenValveOnSecondTrigger.IsChecked ?? false)));
            }
        }
    }
    ...
    {
        if (int.TryParse(txbOpenValveSoundDuration.Text, out int duration) && int.TryParse(txbOpenValveSoundDelay.Text, out int delay))
        {
            int channel = (int)sldOpenValveSound.Value;
            if (cmbOpenValveSound.SelectedIndex == 0)
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValveAfterSound(channel, duration, delay, chkOpenValveSoundSecondTrigger.IsChecked ?? false)));
            }
            else
            {
                HandleResponse(_sniff0.Send(Communicator.Command.OpenValveThenSound(channel, duration, delay, chkOpenValveSoundSecondTrigger.IsChecked ?? false)));
            }
        }
    }
}
